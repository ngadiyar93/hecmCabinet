...
static double Ts = 1.0 / 60000.0; // [sec]
...
int task_mill_init(void)
{
	  if (scheduler_tcb_is_registered(&tcb)) {
		...
		PMSMController_U.V_dc = Vdc;
		PMSMController_U.Id_ref = 0;
		PMSMController_U.T_ref = 0;
...
void task_mill_callback(void *arg)
{
	...
	// call the current controller
	 ...
	 // Update theta
		...
		LOG_Iu = (float) (PMSMController_U.I_a);
		LOG_Iv = (float) (PMSMController_U.I_b);
		LOG_Iw = (float) (PMSMController_U.I_c);

		// execute code
		PMSMController_step();

		// assign duty cycle
		VaDuty = PMSMController_Y.normalizedvoltages[0]/Vdc + 0.5;
		VbDuty = PMSMController_Y.normalizedvoltages[1]/Vdc + 0.5;
		VcDuty = PMSMController_Y.normalizedvoltages[2]/Vdc + 0.5;

		pwm_set_duty(0, VaDuty);
		pwm_set_duty(1, VbDuty);
		pwm_set_duty(2, VcDuty);

		// input angle
		PMSMController_U.theta_m = read_encoder();

		speedRad = PMSMController_Y.omega_e;
		LOG_speed = PMSMController_Y.omega_e*60/(2*PI*polePairs);
}

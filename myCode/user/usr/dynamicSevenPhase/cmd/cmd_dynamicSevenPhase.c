...
void cmd_dynamicSevenPhase_register(void)
{
    ...
    //
    // Here is where you define the base command string: "dynamicSevenPhase"
    // and what function is called to handle command
    commands_cmd_init(&cmd_entry, "dynamicSevenPhase", "RL CC application commands", cmd_help, ARRAY_SIZE(cmd_help), cmd_dynamicSevenPhase);
...
{
    // This function is called every time the user types "dynamicSevenPhase"
    // followed by a space and any number of characters.
    //
    // Example user input: dynamicSevenPhase foo 123
    //
...
//             Treat this case as an invalid command input from user
    ...
    if (argc == 2 && strcmp("init", argv[1]) == 0) {
        if (task_dynamicSevenPhase_init() != SUCCESS) {
            return CMD_FAILURE;
        }
    ...
    if (argc == 3 && strcmp("id1", argv[1]) == 0){
		...
		currentControlDynamic_U.id_ref = id1;
    ...
    if (argc == 3 && strcmp("id3", argv[1]) == 0){
		...
		currentControlDynamic_U.id3_ref = id3;
    ...
    if (argc == 3 && strcmp("id5", argv[1]) == 0){
		...
		currentControlDynamic_U.id5_ref = id5;
	...
	if (argc == 3 && strcmp("iq1", argv[1]) == 0){
		...
		currentControlDynamic_U.iq_ref = iq;
	...
	if (argc == 3 && strcmp("iq3", argv[1]) == 0){
		...
		currentControlDynamic_U.iq3_ref = iq3;
	...
	if (argc == 3 && strcmp("iq5", argv[1]) == 0){
		...
		currentControlDynamic_U.iq5_ref = iq5;

		return CMD_SUCCESS;
	}
	if (argc == 3 && strcmp("phi3", argv[1]) == 0){
			double phi3 = strtod(argv[2], NULL);

			if (phi3 > 359 || phi3 < -359){
				debug_printf("Value too high\n");
				return CMD_INVALID_ARGUMENTS;
			}
			phi3 = phi3*8*PI/180;
			currentControlDynamic_U.Theta_3Offset = phi3;
